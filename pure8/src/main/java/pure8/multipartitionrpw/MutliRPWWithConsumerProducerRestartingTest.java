 ...
 * 
 */
public class MutliRPWWithConsumerProducerRestartingTest {
	public static void main(String[] args) throws InterruptedException, IOException {

		deleteDirectory(new File("/tmp/kafka-logs-3nodemininsync2-0"));
		deleteDirectory(new File("/tmp/kafka-logs-3nodemininsync2-1"));
		deleteDirectory(new File("/tmp/kafka-logs-3nodemininsync2-2"));
		deleteDirectory(new File("/tmp/zookeeper-3nodemininsync2"));

		Injector inj = new Injector();
		try (
			ZookeeperLauncher z = new ZookeeperLauncher(1, "/home/edward/Downloads/kafka_2.11-2.2.1",
					"/home/edward/Documents/kafos/kepsen/3nodemininsync2/zookeeper.properties.",
					(i, s1, s2) -> new ZookeeperProcess(i, s1, s2));
	
			KafkaLauncher l = new KafkaLauncher(3, "/home/edward/Downloads/kafka_2.11-2.2.1",
					"/home/edward/Documents/kafos/kepsen/3nodemininsync2/kafkaserver.properties.",
					(i, s1, s2) -> new KafkaProcess(i, s1, s2));
					){
	
			
			Thread.sleep(5000);
			String boot = "127.0.0.10:9092";
			...
			for (int i = 0; i < 4; i++) {
				ExecProcessor ep = new ExecProcessor(i, "pure8.multipartitionrpw.MultiPartitionRPW",
						"groupa topic0 127.0.0.10:9092,127.0.0.11:9092,127.0.0.12:9092 topic1");
				ep.start();
				eps.add(ep);
			}
			...
			//kafka failure
			l.getProcesses().get(2).killHard();
			l.getProcesses().get(1).killHard();
			l.getProcesses().get(1).start();
