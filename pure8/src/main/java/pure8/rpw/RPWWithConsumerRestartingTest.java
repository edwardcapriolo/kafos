 ...
 * 
 * We start multiple RPW and kill them in an attempt to create duplicate messages. Because
 * we are using the same fencing id we get logically correct but effectively singled
 * threaded processes
 * 
 ...
 * 
 */
public class RPWWithConsumerRestartingTest {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		for (int i = 0; i< 4; i++) {
			ExecProcessor ep= new ExecProcessor(i, "pure8.rpw.RPW", "groupa topic0 localhost:9092 topic1");
			ep.start();
			eps.add(ep);
		}
